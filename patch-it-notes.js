...
notesContent = notesContent.replace(
  /<button className="p-2 bg-indigo-600 text-white rounded text-\[11px\] font-medium flex items-center gap-2  hover:bg-indigo-700 transition-colors">/g,
  `<button onClick={() => navigate(\`/\${isIT ? 'it' : 'seo-gmb'}/\${designation || 'manager'}/\${username || 'ashwini'}/notes/create\`)} className="p-2 bg-indigo-600 text-white rounded text-sm font-medium flex items-center gap-2  hover:bg-indigo-700 transition-colors">`
);
