...
const content = fs.readFileSync('d:/projects/All_IN_One/deals-dashboard/server/routes/activities-notes-routes.js', 'utf8');

const firstCols = extractSelect(content, /\(SELECT\s+n\.id,/, /\s+FROM\s+entity_notes\s+n/);
const secondCols = extractSelect(content, /UNION\s+ALL\s+\(SELECT\s+f\.id,/, /\s+FROM\s+followups\s+f/);
