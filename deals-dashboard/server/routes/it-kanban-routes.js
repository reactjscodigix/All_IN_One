  ...
  const generateSmartFallback = (title = '', description = '', fallbackType = 'description', key = 'WR-101', developers = []) => {
    ...
    if (fallbackType === 'description') {
      ...
      return `
        ...
        <h3 class="font-bold text-sm text-indigo-700">✨ AI Enterprise Improvements (Local Fallback Engine):</h3>
        <h4 class="font-bold text-xs mt-2 text-gray-800">📋 Overview</h4>
        <p>${overview}</p>
        <h4 class="font-bold text-xs mt-2 text-gray-800">👤 User Story</h4>
        <p><em>${userStory}</em></p>
        <h4 class="font-bold text-xs mt-2 text-gray-800">✅ Functional Requirements</h4>
        <ul class="list-disc pl-4 space-y-1 text-gray-700">
          ${requirements.map(req => `<li>${req}</li>`).join('')}
        </ul>
        <h4 class="font-bold text-xs mt-2 text-gray-800">🎯 Acceptance Criteria</h4>
        <ol class="list-decimal pl-4 space-y-1 text-gray-700">
          ${acceptanceCriteria.map(ac => `<li>${ac}</li>`).join('')}
        </ol>
        <h4 class="font-bold text-xs mt-2 text-gray-800">🔧 Technical Notes</h4>
        <ul class="list-disc pl-4 space-y-1 text-gray-700">
          ${technicalNotes.map(n => `<li>${n}</li>`).join('')}
        </ul>
        <h4 class="font-bold text-xs mt-2 text-gray-800">⚠️ Edge Cases</h4>
        <ul class="list-disc pl-4 space-y-1 text-gray-700">
          ${edgeCases.map(ec => `<li>${ec}</li>`).join('')}
        </ul>
        <h4 class="font-bold text-xs mt-2 text-gray-800">🧪 Testing Scenarios</h4>
        <ul class="list-disc pl-4 space-y-1 text-gray-700">
          ${testingScenarios.map(ts => `<li>${ts}</li>`).join('')}
        </ul>
        <p class="text-[10px] text-gray-400 mt-4"><em>(Generated dynamically based on "${cleanTitle}" using CRM local engine rules)</em></p>
      `.trim();
    }
