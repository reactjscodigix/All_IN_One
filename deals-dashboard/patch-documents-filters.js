...
content = content.replace(
  /\{ \['All Projects', 'All Categories', 'All File Types', 'Uploaded By'\]\.map\(\(f, i\) => \(\s*<div key=\{i\} className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-200 rounded-lg cursor-pointer shadow-sm hover:bg-gray-50 transition-colors">\s*<span className="text-\[11px\] font-medium text-gray-700 whitespace-nowrap">\{f\}<\/span>\s*<ChevronDown size=\{14\} className="text-gray-400" \/>\s*<\/div>\s*\)\}/g,
  `<div className="relative">
      <select value={selectedProject} onChange={e => setSelectedProject(e.target.value)} className="pl-3 pr-8 py-2 bg-white border border-gray-200 rounded-lg text-[11px] font-medium text-gray-700 cursor-pointer shadow-sm hover:bg-gray-50 appearance-none focus:outline-none">
        {uniqueProjects.map((p, i) => <option key={i} value={p}>{p}</option>)}
      </select>
      <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
    </div>
    <div className="relative">
      <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} className="pl-3 pr-8 py-2 bg-white border border-gray-200 rounded-lg text-[11px] font-medium text-gray-700 cursor-pointer shadow-sm hover:bg-gray-50 appearance-none focus:outline-none">
        {uniqueCategories.map((c, i) => <option key={i} value={c}>{c}</option>)}
      </select>
      <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
    </div>
    <div className="relative">
      <select value={selectedFileType} onChange={e => setSelectedFileType(e.target.value)} className="pl-3 pr-8 py-2 bg-white border border-gray-200 rounded-lg text-[11px] font-medium text-gray-700 cursor-pointer shadow-sm hover:bg-gray-50 appearance-none focus:outline-none">
...
content = content.replace(
  /<button className="px-3 py-2 text-indigo-600 bg-indigo-50 rounded-lg text-\[11px\] font-medium hover:bg-indigo-100 transition-colors">\s*Reset\s*<\/button>/g,
  `<button onClick={() => { setSearchTerm(''); setSelectedProject('All Projects'); setSelectedCategory('All Categories'); setSelectedFileType('All File Types'); setSelectedFolder('All Documents'); }} className="px-3 py-2 text-indigo-600 bg-indigo-50 rounded-lg text-[11px] font-medium hover:bg-indigo-100 transition-colors">Reset</button>`
);
...
content = content.replace(
  /<div className="flex items-center justify-between p-2 bg-indigo-50\/50 rounded-lg cursor-pointer">/g,
