    ...
    <div className="bg-white rounded  border border-gray-100  flex flex-col">
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-100 rounded-lg text-sm focus:outline-none focus:border-red-500 transition-colors"
              />
            </div>
            <button
              onClick={onAdd}
      ...
      {viewMode === 'kanban' && kanbanView ? (
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 bg-gray-50 overflow-y-auto custom-scrollbar" style={{ maxHeight: '600px' }}>
           {paginatedData.length > 0 ? (
             paginatedData.map((row, idx) => (
               <div key={row.id || idx} className="bg-whitep-2   rounded-lg border border-gray-200  cursor-pointer">
                  <div className="flex items-center justify-between mb-4">
                     <div className="flex items-center gap-3">
                       <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white font-[500]" style={{ backgroundColor: '#10b981' }}>
            ...
            <div className="flex gap-1">
               {[...Array(totalPages)].map((_, i) => (
                 <button
                   ...
                   className={`w-8 h-8 rounded-lg text-xs  transition-colors ${currentPage === i + 1 ? 'bg-red-600 text-white' : 'text-gray-600 hover:bg-gray-50 border border-gray-100'}`}
                 >
                   {i + 1}
                 </button>
               ))}
