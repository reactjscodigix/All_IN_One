...
const SalesReportsPage = () => {
  ...
  const columns = [
      ...
      render: (value, report) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            {report.details && <span className="text-[10px] text-gray-500">{report.details}</span>}
          </div>
        </div>
      )
    },
    ...
    { 
      ...
      sortable: true,
      render: (value) => (
        <span className={`p-1  rounded text-[11px]  ${
          value === 'PDF' ? 'bg-red-50 text-red  border border-red-100' : 'bg-green-50 text-green-600 border border-green-100'
        }`}>
          {value}
        </span>
      )
    ...
    <div className="w-full bg-gray-50 min-h-screen p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {[
          ...
          <button key={idx} className="bg-white p-6 rounded border border-gray-200 shadow-sm hover:border-[#F62416] transition group text-left">
