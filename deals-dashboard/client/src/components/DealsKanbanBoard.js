...
const DealsKanbanBoard = ({ onDealClick, onDealUpdate }) => {
  ...
  const handleConvertToProject = async (e, deal) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/deals/${deal.id}/convert-to-project`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        alert(`Failed to convert: ${errorData.error}`);
      }
    } catch (err) {
      console.error('Error converting to project:', err);
      alert('Error converting deal to project');
    }
  };
                    ...
                    >
                      <div className="flex items-start gap-2 mb-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] text-blue-600  truncate mt-0.5">{deal.service}</p>
