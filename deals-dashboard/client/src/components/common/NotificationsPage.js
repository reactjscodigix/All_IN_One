          ...
          {KPIS.map(kpi => (
            <button key={kpi.label} onClick={() => setActiveTab(kpi.tab)}
              className={`bg-white rounded p-4 border text-left transition-all hover:shadow-md
                ${activeTab === kpi.tab ? 'border-blue-300 ring-1 ring-blue-200 shadow-sm' : 'border-gray-100 shadow-sm'}`}>
        ...
        <div className="bg-white rounded border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-4">
            ...
            <div className="ml-auto flex items-center gap-2">
              <button onClick={markAllRead}
                className="flex items-center gap-1.5 px-3 py-2 text-[12px]  text-gray-600 hover:bg-gray-50 rounded border border-gray-200 transition-colors">
                <CheckCheck size={14} className="text-green-500" /> Mark all as read
              </button>
              <button className="flex items-center gap-1.5 px-3 py-2 text-[12px]  text-gray-600 hover:bg-gray-50 rounded border border-gray-200 transition-colors">
          ...
          <div>
            {Object.entries(byDate).map(([date, items]) => (
              <div key={date}>
                <div className="px-5 py-2 text-xs font-extrabold text-gray-500 uppercase tracking-wider bg-gray-50 border-b border-gray-100">{date}</div>
                {items.map(notif => {
              ...
              {[1, 2, 3, 4, 5].map(p => (
                <button key={p} onClick={() => setCurrentPage(p)}
                  className={`w-7 h-7 flex items-center justify-center rounded text-[12px]  transition-colors
                    ${currentPage === p ? 'bg-red-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-200'}`}>{p}</button>
              ))}
            ...
            <div className="relative ml-auto w-[310px] bg-white shadow-2xl border-l border-gray-200 flex flex-col h-full overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto">
                ...
                <div className="flex flex-col items-center py-6 border-b border-gray-100">
                  <div className={`w-16 h-16 rounded-2xl ${cfg.color} flex items-center justify-center mb-3 shadow-sm`}>
                ...
                <div className="px-5 py-4 border-b border-gray-100">
                  <h5 className="text-xs font-extrabold text-gray-700 uppercase tracking-wider mb-3">Details</h5>
                ...
                {/* Additional Info */}
                <div className="px-5 py-4 border-b border-gray-100">
                  <h5 className="text-xs font-extrabold text-gray-700 uppercase tracking-wider mb-3">Additional Information</h5>
                  <div className="space-y-2.5">
                ...
                {/* Actions */}
                <div className="px-5 py-4 border-b border-gray-100">
                  <h5 className="text-xs font-extrabold text-gray-700 uppercase tracking-wider mb-3">Actions</h5>
