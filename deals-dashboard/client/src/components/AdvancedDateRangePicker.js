...
const AdvancedDateRangePicker = ({ range, onChange, initialRange = 'Today' }) => {
      ...
      <div className="w-64">
        ...
        <div className="grid grid-cols-7 gap-1">
          {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map(d => (
            <div key={d} className="h-8 w-8 flex items-center justify-center text-[10px]  text-gray-400">
              {d}
            </div>
          ))}
          {days}
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute left-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-xl z-[100] flex overflow-hidden">
          {/* Presets */}
