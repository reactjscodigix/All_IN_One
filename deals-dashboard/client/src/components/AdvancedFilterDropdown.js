  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-2 px-3 py-2   border border-gray-200 rounded text-xs transition-colors hover:bg-red-600 hover:text-white  "
      >
        <Filter size={14} />
        <span className="">Filter</span>
        {activeFilterCount > 0 && (
          <span className="flex items-center justify-center w-4 h-4 bg-white text-red  text-[10px] rounded-full font-[500] ml-1">
            {activeFilterCount}
          </span>
        )}
      ...
      {isOpen && (
        <div className="absolute left-0 mt-2 w-72 bg-white border border-gray-200 rounded-lg shadow-xl z-[100] flex flex-col overflow-hidden">
          {/* Header */}
