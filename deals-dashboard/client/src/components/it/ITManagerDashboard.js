    ...
    <div className="p-6 bg-[#F8FAFC] min-h-screen font-sans">
      ...
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => navigate(urlPrefix + '/projects')} className="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-medium flex items-center gap-2 hover:bg-indigo-700 transition-colors">
            <Plus size={16} />
            New Project
          </button>
          <div className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-200 rounded-md ml-2 cursor-pointer shadow-sm">
      ...
      <div className="grid grid-cols-6 gap-4 mb-6">
        {kpiData.map((kpi, idx) => (
          <div key={idx} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow cursor-pointer">
            ...
            <div>
              ...
              <div className={`text-[10px] font-medium ${kpi.trendColor}`}>
                {kpi.trend}
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-12 gap-6 mb-6">
        ...
        <div className="col-span-3 bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
          ...
          <div className="mt-6 pt-4 border-t border-gray-100 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <span className="px-3 py-1 bg-green-50 text-green-600 text-[10px]  rounded-full border border-green-100">Good</span>
            ...
            {projectsTimeline.map((proj, idx) => (
              <div key={idx} className="flex items-center w-full z-10 group">
                <div className="w-48 shrink-0 pr-4">
                  ...
                  <div className="text-[10px] text-gray-500">Progress {proj.progress}%</div>
                </div>
            ...
            {activityFeed.map((activity, idx) => (
              <div key={idx} className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10px] font-medium text-gray-400">{activity.project}</div>
                </div>
                <div className="text-[10px] text-gray-400 shrink-0 whitespace-nowrap">{activity.time}</div>
              </div>
            ))}
          </div>
        </div>
            ...
            {[
              ...
              <div key={idx} className="flex items-center justify-between group cursor-pointer">
                <div className="flex items-center gap-2 w-28">
                  <div className={`p-1.5 rounded-lg ${stat.iconBg} text-gray-500 group-hover:text-indigo-600 transition-colors`}>
            ...
            {teamWorkload.map((member, idx) => (
              <div key={idx} className="flex items-center justify-between group cursor-pointer">
                ...
                <div className="flex flex-col items-end w-24">
                  <div className="text-[10px]  text-gray-700 mb-1">{member.progress}%</div>
          ...
          <div className="flex-1 flex flex-col justify-center">
            <div className="relative h-40 w-full mb-4">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-[10px] text-gray-500">Total Hours</span>
              </div>
            </div>
              ...
              {timeTrackingData.map((item, idx) => (
                <div key={idx} className="flex items-center justify-between">
                  ...
                  <div className="flex flex-col items-end">
                    <span className="text-[10px]  text-gray-900">{item.value}h</span>
            ...
            {topOpenTasks.map((task, idx) => (
              <div key={idx} className="group cursor-pointer">
                ...
                <div className="flex justify-between items-center">
                  <div className="text-[10px] text-gray-500">{task.project}</div>
                  <div className="text-[10px] font-medium text-gray-400">Due: {task.due}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* FOOTER BAR */}
      <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between overflow-x-auto gap-8 whitespace-nowrap">
        <div className="flex items-center gap-3 min-w-[120px]">
          <div className="p-1.5 rounded-lg bg-indigo-50 text-indigo-600"><Activity size={16} /></div>
          <div>
            <div className="text-[10px] text-gray-500 font-medium">Active Sprints</div>
            <div className="text-sm  text-gray-900">5</div>
          </div>
        </div>

        <div className="flex items-center gap-3 min-w-[140px]">
          <div className="p-1.5 rounded-lg bg-blue-50 text-blue-600"><Flag size={16} /></div>
          <div>
            <div className="text-[10px] text-gray-500 font-medium">Upcoming Milestones</div>
            <div className="text-sm  text-gray-900">7</div>
          </div>
        </div>

        <div className="flex items-center gap-3 min-w-[140px]">
          <div className="p-1.5 rounded-lg bg-sky-50 text-sky-600"><CheckCircle size={16} /></div>
          <div>
            <div className="text-[10px] text-gray-500 font-medium">Client Approvals</div>
            <div className="text-sm  text-gray-900">3 <span className="text-[10px] font-medium text-gray-400">Pending</span></div>
          </div>
        </div>

        <div className="flex items-center gap-3 min-w-[130px]">
          <div className="p-1.5 rounded-lg bg-rose-50 text-rose-500"><Bug size={16} /></div>
          <div>
            <div className="text-[10px] text-gray-500 font-medium">Unresolved Bugs</div>
            <div className="text-sm  text-gray-900">42</div>
          </div>
        </div>

        <div className="flex items-center gap-3 min-w-[120px]">
          <div className="p-1.5 rounded-lg bg-amber-50 text-amber-500"><Clock size={16} /></div>
          <div>
            <div className="text-[10px] text-gray-500 font-medium">Overdue Tasks</div>
        ...
        <div className="flex-1 flex justify-end">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 rounded-full border border-gray-100">
            <div className="text-[10px] text-gray-500 font-medium">System Status</div>
            <div className="flex items-center gap-1.5">
              <div className="w-2 h-2 rounded-full bg-green-500"></div>
              <div className="text-[10px]  text-gray-900">All Systems Operational</div>
            </div>
          </div>
        </div>
      </div>
