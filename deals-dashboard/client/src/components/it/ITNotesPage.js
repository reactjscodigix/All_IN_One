...
function Avatar({ name, size = 28 }) {
  return (
    <img src={AVATARS[name]} alt={name} className="rounded-full object-cover shrink-0 border-2 border-white"
    ...
    <div className="flex h-screen bg-[#f8fafc] font-sans text-gray-900 overflow-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="w-[230px] shrink-0 bg-white border-r border-gray-100 flex flex-col overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-100">
          <button onClick={() => navigate(`/${isIT ? 'it' : 'seo-gmb'}/${designation || 'manager'}/${username || 'ashwini'}/notes/create`)} className="w-full flex items-center justify-between px-3 py-2 bg-gray-50 hover:bg-blue-50 border border-gray-200 rounded text-[12px]  text-gray-700 hover:text-blue-600 hover:border-blue-200 transition-all group">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1">
          ...
          {NAV_ITEMS.map(item => (
            <button key={item.label} onClick={() => { setSelectedNav(item.label); setSelectedCategory(null); }}
              className={`w-full flex items-center justify-between px-3 py-2 rounded text-[12px]  transition-all
              ...
              <span className={`text-xs  ${selectedNav === item.label && !selectedCategory ? 'text-red-400' : 'text-gray-400'}`}>{item.count}</span>
            </button>
          ))}
          ...
          <div className="pt-3">
            <div className="flex items-center justify-between px-3 py-1.5 mb-1">
              <span className="text-xs font-extrabold text-gray-400 uppercase tracking-wider">Categories</span>
            ...
            {CATEGORIES.map(cat => (
              <button key={cat.label} onClick={() => { setSelectedCategory(cat.label); setSelectedNav(''); }}
                ...
                <span className="text-[9px]  text-gray-400 shrink-0">{cat.count}</span>
              </button>
            ))}
          </div>
          ...
          <div className="pt-3">
            <div className="px-3 py-1.5 mb-2">
              <span className="text-xs font-extrabold text-gray-400 uppercase tracking-wider">Tags</span>
            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            {['All Content', 'Notes', 'Wiki Pages'].map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`px-3 py-1.5 text-[12px]  rounded transition-colors border
                  ${activeTab === tab ? 'bg-red-600 text-white border-red-600 shadow-sm' : 'text-gray-600 border-gray-100 hover:bg-gray-50'}`}>
                {tab}
              </button>
            ))}
          </div>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className="relative">
              <button onClick={() => setShowSortMenu(p => !p)}
                className="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded bg-white text-[12px]  text-gray-700 hover:bg-gray-50 shadow-sm">
        ...
        <div className="flex-1 overflow-y-auto" onClick={() => setShowSortMenu(false)}>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  <th className="pl-6 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wider">Title</th>
                  <th className="px-4 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wider">Type</th>
                  <th className="px-4 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wider">Category</th>
                  <th className="px-4 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wider">Updated By</th>
                  <th className="px-4 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wider">Updated At</th>
          ...
          <div className="bg-white border-t border-gray-100 px-6 py-3 flex justify-between items-center">
            <span className="text-xs  text-gray-500">Showing 1 to 10 of 128 entries</span>
            <div className="flex items-center gap-1">
              ...
              {[1, 2, 3, 4, 5].map(p => (
                <button key={p} className={`w-7 h-7 flex items-center justify-center rounded text-[12px]  ${p === 1 ? 'bg-red-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'}`}>{p}</button>
              ))}
        ...
        <div className="w-[270px] shrink-0 bg-white border-l border-gray-100 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="flex justify-center py-5 border-b border-gray-100">
              <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-sm ${selectedNote.type === 'Wiki Page' ? 'bg-blue-100' : 'bg-green-100'}`}>
            ...
            <div className="px-4 py-4 border-b border-gray-100">
              <h4 className="text-xs font-extrabold text-gray-700 uppercase tracking-wider mb-2">Description</h4>
            ...
            <div className="px-4 py-4 border-b border-gray-100 space-y-2">
              <button className="w-full flex items-center justify-center gap-2 py-2.5 bg-red-600 text-white text-[12px]  rounded hover:bg-red-700 transition-colors shadow-sm">
            ...
            {selectedNote.recentInCategory?.length > 0 && (
              <div className="px-4 py-4 border-b border-gray-100">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="text-xs font-extrabold text-gray-700 uppercase tracking-wider">Recent Pages in this Category</h4>
            ...
            {selectedNote.attachments?.length > 0 && (
              <div className="px-4 py-4">
                <h4 className="text-xs font-extrabold text-gray-700 uppercase tracking-wider mb-3">Attachments ({selectedNote.attachments.length})</h4>
