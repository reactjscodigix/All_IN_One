  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-800 rounded px-3 py-1.5 text-xs  shadow-sm cursor-pointer transition-colors"
      >
      ...
      {isOpen && (
        <div className="absolute left-0 mt-1.5 w-64 bg-white border border-slate-200 rounded shadow-lg py-1.5 z-50 text-xs">
          <div className="px-3 py-1.5 border-b border-slate-100  text-slate-400 text-xs uppercase tracking-wider">
            Switch Project
          </div>
            ...
            {projects.map((project) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 flex items-center justify-between hover:bg-slate-50 border-none bg-transparent cursor-pointer font-medium ${isSelected ? 'text-blue-600 bg-blue-50/50' : 'text-slate-700'
                    }`}
                >
