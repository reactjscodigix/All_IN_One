        ...
        <div className="flex justify-between items-center px-4 py-3 border-b border-[#EAECF0] sticky top-0 bg-white z-10">
          <div className="flex flex-col">
            ...
            {initialData?.related_name && (
              <span className="text-[10px] text-gray-500 ">FOR: {initialData.related_name.toUpperCase()}</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            {!formData.id && (
              <span className={`px-2 py-0.5 rounded text-[9px]    ${
                formData.isNextFollowup ? 'bg-purple-50 text-purple-600 border border-purple-100' : 
              ...
              {openPanels.basic && (
                <div className="p-2 space-y-2 bg-white animate-in fade-in duration-200">
                  {formData.previous_outcome && (
                    <div className="p-2 mb-2 bg-blue-50 border border-blue-100 rounded-lg flex items-start gap-3 animate-in slide-in-from-top-2 duration-300">
                      <div className="w-6 h-6 flex-shrink-0 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center  text-[10px]">
                        i
                      </div>
                      <div className="flex flex-col gap-0.5">
                        <span className="text-[10px]   text-blue-600 ">Previous Outcome</span>
                  ...
                  <div>
                    <label className="block text-xs  text-gray-700 mb-1.5 flex justify-between items-center">
                      ...
                      {formData.isNextFollowup ? (
                        <span className="text-[10px] text-purple-500  animate-pulse">Update subject for this follow-up</span>
                      ) : isFirstForClient ? (
                        <span className="text-[10px] text-green-500  animate-pulse">First follow-up for this client</span>
                      ) : null}
                    </label>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <select
                          ...
                          onChange={handleInputChange}
                          className={`w-full p-2 border rounded text-xs appearance-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 outline-none transition-all pr-10 ${
                            formData.priority === 'High' ? 'border-red-200 text-red-700 bg-red-50' : 
                  ...
                  {isMeetingType && (
                    <div className="p-2 bg-blue-50/50 rounded border border-blue-100/50 animate-in slide-in-from-top-2 duration-300 space-y-3">
                      {['Google Meet', 'Zoom Meeting', 'Demo', 'Internal Video Call', 'WhatsApp Call'].includes(formData.type) && (
                        <div>
                          <label className="block text-[10px]  text-blue-600   mb-1.5 flex items-center gap-2">
                            <VideoIcon size={12} />
